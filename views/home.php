<?php
$page_title = "Welcome to my home page";
?>
<div class="container-fluid">
<div class="row">
<div class="col-sm-12 background-color">
<h1 class="h1-size-1">Welcome to the Vanarts Student Portal, Vikrant</h1>
</div>
<div class="col-sm-12 background-color">
  <a href="#">
  <h1 class="h1-size-2">87&#37;</h1>
  <h2 class="h2-size-1">Average</h2>
  </a>
  </div>
  <div class="col-sm-12 background-color">
      <a href="#">
      <h1 class="h1-size-3">6&#37;</h1>
      <h2 class="h2-size-2">Pending</h2>
      <h2 class="h2-size-2">Assignments</h2>
      </a>
      </div>
       <div class="col-sm-12 background-color">
              <a href="#">
              <h1 class="h1-size-4">6&#37;</h1>
              <h2 class="h3-size-3">Submitted</h2>
              <h2 class="h3-size-3">Assignments</h2>
              </a>
              </div>
              <div class="col-sm-12 background-color">
                  <a href="#">
                  <img src="assets/icon1.jpg" alt="hat-icon" class="img-size"/>
                  <h1 class="h1-size-5">courses</h1>
                  </a>
                  </div>
                  <div class="col-sm-12 background-color">
                      <a href="#">
                      <img src="assets/icon1.jpg" alt="hat-icon" class="img-size"/>
                      <h1 class="h1-size-5">courses</h1>
                      </a>
                      </div>
</div>
</div>